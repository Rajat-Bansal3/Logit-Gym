...
export class GymRepository {
	...
	async create(data: { name: string; address: string; ownerId: string }): Promise<Gym> {
	...
	async createProfile(gymId: string, profile: NonNullable<CreateGym["profile"]>): Promise<void> {
	...
	async upsertProfile(gymId: string, profile: UpdateGym["profile"]): Promise<void> {
		if (!profile) {
			return;
		}
